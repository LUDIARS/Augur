...
2. **Normalize** — parse raw signals into uniform internal facts: split diffs per file, extract stack frames from failure logs, parse coverage content by its declared format, and tag runtime signals.
...
## LLM Enrichment (Future, Optional)

A later phase may add an enrichment stage that runs **after** assembly:

- It may rewrite prose fields only: `summary`, `rationale`, `draft.description`, `draft.outline`, and step `description`s.
- It must not add, remove, or reorder suggestions, fix steps, risks, or evidence, and must not change ids, kinds, priorities, confidences, or strategies.
- It is controlled by `AUGUR_LLM_PROVIDER` / `AUGUR_LLM_API_KEY` and is disabled when they are unset.
- Golden and safety tests always run with enrichment disabled.
