...
`planId` and `createdAt` are stamped by the CLI layer after the engine returns, never by the engine itself; see [Plan Persistence](./persistence.md). With persistence disabled they are absent and the response is identical to the stateless behavior.
